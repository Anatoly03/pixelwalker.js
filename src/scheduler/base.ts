...
import Client from "../lobby";
import Connection from "../connection";
import wait from "../util/wait";

export class SchedulerEntry<V> {
...
export default abstract class BaseScheduler<K extends string, V> {
    ...
    private async loop() {
        if (!this.connection.connected()) return this.stop();
        ...
        for (const entry of entries) {
            ...
            if (this.INBETWEEN_DELAY != 0) await wait(this.INBETWEEN_DELAY);
        }
    ...
    public add(k: K, el: V, priority?: number): Promise<boolean> {
        if (!this.connection.connected() || !this.running) return Promise.resolve(this.stop());
        ...
        const promise = new Promise((res: (v: boolean) => void, rej: (v: any) => void) => {
            if (!this.connection.connected() || !this.running) return res(false);
